...
import { Plus, Search, MoreVertical, Edit, Trash2, Package, Eye, ChevronLeft, ChevronRight } from "lucide-react";
...
import { formatCost, formatCostShort, getDaysUntilRenewal, getRenewalUrgency, getMonthlyEquivalentINR, getAnnualContributionINR, getStatusVariant, SUB_QUERY_KEYS } from "@/lib/subscriptionUtils";
...
export const ToolsList = () => {
  ...
  const paginatedTools = sortedData.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE);
          ...
          <TableBody>
            {isLoading ? (
              ...
              >
                ...
                <TableCell className="text-sm text-right py-2">{tool.quantity || 1}</TableCell>
                <TableCell className="text-sm text-right py-2">{formatCost(tool.unit_cost, tool.currency)}</TableCell>
                <TableCell className="text-sm font-medium text-right py-2">{formatCost(tool.total_cost, tool.currency)}</TableCell>
                <TableCell className="text-sm py-2">{(tool.subscriptions_vendors as any)?.name || "—"}</TableCell>
                <TableCell className="py-2">
                  {tool.renewal_date ? (
