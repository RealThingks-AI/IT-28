...
export function DeniedAssetsDialog({ open, onOpenChange }: DeniedAssetsDialogProps) {
  ...
  const { data: deniedAssets = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("itam_assets")
        .select("id, name, asset_tag, asset_id, status, assigned_to, confirmation_status, last_confirmed_at, category:itam_categories(name)")
    ...
    mutationFn: async (assetId: string) => {
      ...
      if (user) {
        ...
        await supabase.from("itam_asset_history").insert({
          ...
          new_value: "Available", details: { reason: "denied_by_user" },
  ...
  const reassignMutation = useMutation({
    mutationFn: async ({ assetId, newUserId }: { assetId: string; newUserId: string }) => {
      const { data: { user } } = await supabase.auth.getUser();
      await supabase.from("itam_assets").update({
        assigned_to: newUserId, status: "in_use", confirmation_status: null,
    ...
    mutationFn: async (assetId: string) => {
      ...
      await supabase.functions.invoke("send-asset-email", {
        body: {
          ...
          assets: [{ asset_tag: asset.asset_id || asset.asset_tag || "N/A", description: (asset as any).category?.name || asset.name || "N/A" }],
          variables: {
              ...
              {isLoading ? (
                ...
                deniedAssets.map((asset: any) => (
                  <TableRow key={asset.id} className={`hover:bg-muted/50 transition-colors ${selectedIds.has(asset.id) ? "bg-primary/5" : ""}`}>
                    ...
                    <TableCell><code className="text-xs bg-muted px-1.5 py-0.5 rounded font-mono">{asset.asset_id || asset.asset_tag || "—"}</code></TableCell>
                    <TableCell className="text-sm font-medium">{asset.name || "—"}</TableCell>
                    <TableCell className="text-sm">{asset.category?.name || "—"}</TableCell>
                    <TableCell className="text-sm">{asset.assigned_to ? (userMap.get(asset.assigned_to) || "—") : "—"}</TableCell>
                    <TableCell>
