...
import {
  ...
  Calendar, Clock, KeyRound, Trash2, ShieldCheck, XCircle,
} from "lucide-react";
      ...
      {enabledWidgets.map((widget, index) => {
        ...
        switch (widget.id) {
          ...
          case "pendingConfirmation":
            return <AssetStatCard key={widget.id} title="Pending Confirm" value={pendingConfirmationCount} subtitle="Overdue for verification" icon={ShieldCheck} iconBgColor="bg-sky-500" iconColor="text-white" onClick={() => navigate("/assets/allassets?confirmation=overdue")} animationDelay={d} />;
          case "deniedAssets":
            return <AssetStatCard key={widget.id} title="Denied" value={deniedCount} subtitle="Needs immediate action" icon={XCircle} iconBgColor="bg-red-600" iconColor="text-white" onClick={() => navigate("/assets/allassets?confirmation=denied")} animationDelay={d} />;
          default:
            return null;
        }
      })}
