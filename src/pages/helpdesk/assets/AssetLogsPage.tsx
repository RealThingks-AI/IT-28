    ...
    <ScrollArea className="h-full">
      <div className="p-4 space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: "Total Logs", value: total, icon: ClipboardList, colorClass: "bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400" },
            { label: "Check Outs", value: statCounts?.checkouts ?? "—", icon: LogOut, colorClass: "bg-yellow-100 text-yellow-600 dark:bg-yellow-900/30 dark:text-yellow-400" },
            { label: "Check Ins", value: statCounts?.checkins ?? "—", icon: LogIn, colorClass: "bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400" },
            { label: "Status Changes", value: statCounts?.changes ?? "—", icon: Activity, colorClass: "bg-purple-100 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400" },
          ].map((s) => (
            <Card key={s.label}>
        ...
        <div className="flex flex-wrap gap-2">
          <div className="relative max-w-sm flex-1 min-w-[200px]">
            ...
            <Input placeholder="Search logs..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(0); }} className="pl-9 h-9" />
          </div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow><TableCell colSpan={6} className="text-center py-12">
                  <div className="flex flex-col items-center justify-center">
                    <ClipboardList className="h-12 w-12 text-muted-foreground mb-3 opacity-50" />
