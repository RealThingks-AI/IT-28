...
function replacePlaceholders(text: string, variables: Record<string, string>): string {
  ...
  for (const [key, value] of Object.entries(variables)) {
    if (key === "confirm_all_url" && value) {
      ...
      const btn = `<a href="${value}" style="display:inline-block;padding:10px 24px;background:#dc2626;color:#ffffff;font-weight:600;font-size:13px;text-decoration:none;border-radius:6px;margin:4px 8px 4px 0;">&#10060; Deny All</a>`;
      result = result.replace(new RegExp(`{{${key}}}`, "g"), btn);
    } else {
